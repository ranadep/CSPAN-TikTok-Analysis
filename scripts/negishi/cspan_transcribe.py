"""Transcribe a snapshot of completed C-SPAN downloads; rerun for new files."""
...
OUT = ROOT / 'transcripts-small.en'
MODEL = None

def initialize(model_path):
    global MODEL
    from faster_whisper import WhisperModel
    MODEL = WhisperModel(model_path, device='cpu', compute_type='int8', cpu_threads=4, num_workers=1)
...
def transcribe(item):
    ...
    try:
        ...
        rows = [{'start': s.start, 'end': s.end, 'text': s.text,
                ...
                for s in segments]
        result = {'source_file': str(video), 'source_url': url, 'model': 'small.en',
...
def main():
    ...
    print('Preparing small.en model (first run downloads weights).', flush=True)
    model_path = download_model('small.en')
    workers = min(32, max(1, int(os.environ.get('SLURM_CPUS_PER_TASK', '128')) // 4), len(pending))
    print(f'Starting {workers} workers, 4 CPU threads each.', flush=True)
    ...
    print('Snapshot finished. Rerun to pick up newly downloaded videos or retry errors.', flush=True)
